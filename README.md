# 32-bit RISC-V Single-Cycle Processor ASIC Implementation

This project implements and analyzes a 32-bit RISC-V single-cycle processor using a complete ASIC design flow. The design was verified using RTL simulation, synthesized using Synopsys Design Compiler, analyzed using PrimeTime, and implemented through backend layout generation using IC Compiler.

## Project Overview

The objective of this project was to evaluate the ASIC implementation efficiency of a 32-bit RISC-V single-cycle processor across two standard-cell technology nodes:
...
The processor is a single-cycle RISC-V core where each instruction completes in one clock cycle. The design includes the following major modules:
...
The SAED90nm backend flow successfully generated post-route outputs including .v, .spef, and .gds.
Conclusion

This project demonstrates a complete ASIC implementation and analysis flow for a 32-bit RISC-V single-cycle processor. The design was successfully verified, synthesized, timing-analyzed, and implemented through backend layout generation.
